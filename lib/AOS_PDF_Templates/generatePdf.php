 ...
 */
	...
	if($task == 'pdf' || $task == 'emailpdf')
	{
		...
		try{
			//==================================================================================================
			//MODIFICACION PARA COLOCAR LEYENDA DE CANCELADO A REMISIONES CANCELADAS, DUH!
			
			$sqlCanc = "SELECT cancelar_c FROM aos_invoices_cstm WHERE id_c = '".$module->id."'";
			$resCanc = $module->db->query($sqlCanc);
			$filasCanc = $module->db->getRowCount($resCanc);
			if($filasCanc>0){
				$rowCanc = $module->db->fetchByAssoc($resCanc);
			}
			...
			if($filasCanc>0){
				if($rowCanc['cancelar_c']=='1'){
					$pdf->SetWatermarkText('CANCELADO');
